...
export default class AdAccountService {
    /**
    ...
    private async getData() {
        ...
        let page = 1
        let allResults: any[] = []
        let currentResult: any = await (await axios.get(`${backendBaseUrl}/api/ad_accounts`)).data.meta
    ...
    /**
     * Sync Ad Accounts (can be called from Seeder or Controller)
     */
    public async syncAdAccounts() {
        try {
            console.log('⏳ Fetching ad accounts...')
            const adAccounts = await this.getData()

            if (adAccounts.length === 0) {
                console.log('⚠️ No ad accounts found or data already seeded')
                return
            }
